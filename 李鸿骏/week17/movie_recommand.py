...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    pred_score = 0
    # 用户喜爱的电影（打分 3 分以上）
    like_items = [(like_item_id + 1, score) for like_item_id , score in enumerate(user_to_rating[user_id]) if score > 3]
    for like_item_id, score in like_items:
        like_similar_items = similar_items.get(like_item_id,[])
        for sililar_item_id, siliary in like_similar_items:
            if sililar_item_id == item_id:
                pred_score = score * siliary
                return pred_score
    return pred_score
